...
export const HospitalView = () => {
  const { hospitals, ambulances, updatePatientStatus } = useApp();

  const currentHospital = hospitals[0]; // Metropolitan Central Hospital
  const incomingAmbulances = ambulances.filter(a => a.status === 'EN_ROUTE');
        ...
        <div className="flex items-center space-x-3.5">
          ...
          <div>
            <div className="flex items-center space-x-2">
              <h2 className="text-2xl font-black text-white tracking-tight">🏥 {currentHospital.name}</h2>
            ...
            <p className="text-xs text-slate-300 font-semibold mt-0.5">{currentHospital.address} • Trauma Level 1 Center</p>
          </div>
        </div>
                ...
                <div key={amb.id} className="bg-slate-950/90 border border-slate-800 p-5 rounded-2xl space-y-4 shadow-xl">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <h4 className="text-xl font-black text-white mt-0.5">{amb.patient.name}</h4>
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-xl font-black text-emerald-400 font-mono">~{amb.etaMinutes} mins</div>
                    </div>
                  </div>
                  ...
                  <div className="pt-2 border-t border-slate-800 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        onClick={() => updatePatientStatus(amb.patient.id, 'HOSPITAL_PREPARED')}
                      ...
                      <button
                        onClick={() => updatePatientStatus(amb.patient.id, 'ARRIVED')}
                      ...
                      <button
                        onClick={() => updatePatientStatus(amb.patient.id, 'IN_OT')}
                      ...
                      <button
                        onClick={() => updatePatientStatus(amb.patient.id, 'DISCHARGED')}
